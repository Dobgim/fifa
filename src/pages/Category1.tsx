...
const Category1: React.FC = () => {
  ...
  const maxPriceLimit = Math.max(...MATCH_SCHEDULE.map((m) => getTicketPrice(m.basePrice, 1)));
  ...
  const filteredMatches = useMemo(() => {
    return MATCH_SCHEDULE.filter((match) => {
      const ticketPrice = getTicketPrice(match.basePrice, 1);
      ...
      const priceA = getTicketPrice(a.basePrice, 1);
      const priceB = getTicketPrice(b.basePrice, 1);
    ...
    <div className="pt-24 min-h-screen">
      <section className="relative bg-slate-950 py-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto text-center flex flex-col items-center">
          <span className="px-3.5 py-1 rounded-full bg-yellow-500/15 border border-yellow-500/30 text-xs font-bold text-yellow-400 tracking-widest uppercase mb-4">
            🏆 PREMIUM SEATS
          </span>
          ...
          <p className="mt-4 max-w-2xl text-slate-400 text-sm sm:text-base leading-relaxed">
            Prime sideline & lower tier seats with direct tactical sightlines — the closest seats to the pitch available on the secondary market.
          </p>
          <div className="mt-4 flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/20 px-5 py-2 rounded-xl text-yellow-400 text-xs font-bold">
            From <span className="text-xl font-black">${Math.min(...MATCH_SCHEDULE.map(m => getTicketPrice(m.basePrice, 1)))}</span> / ticket
          </div>
        </div>
      </section>
          ...
          <div className="w-full lg:w-3/4 flex flex-col">
            <div className="flex flex-wrap items-center justify-between gap-4 bg-slate-950/50 border border-white/5 p-4 rounded-2xl mb-8">
              <div className="text-sm font-semibold text-slate-400">
                Showing <span className="text-white font-extrabold">{filteredMatches.length}</span> Category 1 matches
              </div>
            ...
            {paginatedMatches.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {paginatedMatches.map((match) => <ProductCard key={match.id} match={match} category={1} />)}
              </div>
            ) : (
