...
const Category3: React.FC = () => {
  ...
  const filteredMatches = useMemo(() => {
    return MATCH_SCHEDULE.filter((match) => {
      ...
      // Price filter (Base price is for Category 3)
      const matchesPrice = match.basePrice <= filters.maxPrice;
      ...
      return a.dateOffset - b.dateOffset; // Default date-asc
    });
  }, [filters, sortBy]);
  ...
  const maxPriceLimit = Math.max(...MATCH_SCHEDULE.map((m) => m.basePrice));
      ...
      <section className="relative bg-slate-950 py-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto text-center flex flex-col items-center">
          ...
          <p className="mt-4 max-w-2xl text-slate-400 text-sm sm:text-base leading-relaxed">
            Enjoy world-class stadium actions behind the goals and in the corner sections at the most budget-friendly rates. Experience the soccer history live!
          </p>
        </div>
      </section>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-3/4 flex flex-col">
            ...
            {/* Grid Controls (Sorting & Result Count) */}
